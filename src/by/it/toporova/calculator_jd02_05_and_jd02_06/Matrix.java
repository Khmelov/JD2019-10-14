...
public class Matrix extends Var {
    ...
    public Var add(Var other) throws CalcException {
        if (other instanceof Scalar) {
            ...
            if (this.value.length != ((Matrix) other).value.length || this.value[0].length != ((Matrix) other).value[0].length) {
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_MATRIX_ADD));
            }
            double[][] res = new double[this.value.length][this.value[0].length];
            for (int i = 0; i < res.length; i++) {
                for (int j = 0; j < res[0].length; j++) {
    ...
    public Var sub(Var other) throws CalcException {
        if (other instanceof Scalar) {
            ...
            if (this.value.length != ((Matrix) other).value.length || this.value[0].length != ((Matrix) other).value[0].length) {
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_MATRIX_SUB));
            }
            double[][] res = new double[this.value.length][this.value[0].length];
            for (int i = 0; i < res.length; i++) {
                for (int j = 0; j < res[0].length; j++) {
    ...
    public Var mul(Var other) throws CalcException {
        if (other instanceof Scalar) {
            ...
            if (this.value[0].length != ((Vector) other).getValue().length) {
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_MATRIX_MUL_VECTOR));
            }
            ...
            if (this.value.length != ((Matrix) other).value.length || this.value[0].length != ((Matrix) other).value[0].length) {
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_MATRIX_MUL));
            }
