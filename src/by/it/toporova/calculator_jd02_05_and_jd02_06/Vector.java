...
public class Vector extends Var {
    ...
    public Var add(Var other) throws CalcException {
        if (other instanceof Scalar) {
            ...
            if (this.value.length != ((Vector) other).value.length)
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_VECTOR_ADD));
            double[] res = Arrays.copyOf(this.value, this.value.length);
            for (int i = 0; i < res.length; i++) {
    ...
    public Var sub(Var other) throws CalcException {
        if (other instanceof Scalar) {
            ...
            if (this.value.length != ((Vector) other).value.length)
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_VECTOR_SUB));
            double[] res = Arrays.copyOf(this.value, this.value.length);
            for (int i = 0; i < res.length; i++) {
                res[i] -= ((Vector) other).value[i];
            }
    ...
    public Var mul(Var other) throws CalcException {
        if (other instanceof Scalar) {
            ...
            if (this.value.length != ((Vector) other).value.length) {
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_VECTOR_MUL));
            }
            double[] res = Arrays.copyOf(this.value, this.value.length);
            double mulResult = 0;
            for (int i = 0; i < res.length; i++) {
    ...
    public Var div(Var other) throws CalcException {
        if (other instanceof Scalar) {
            if (((Scalar) other).getValue()==0)
                throw new CalcException(ResMan.INSTANCE.get(Errors.ERROR_DIV_ZERO));
            double[] res = Arrays.copyOf(this.value, this.value.length);
            for (int i = 0; i < res.length; i++) {
                res[i] /= ((Scalar) other).getValue();
            }
